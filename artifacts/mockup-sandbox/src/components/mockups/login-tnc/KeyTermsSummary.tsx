...
import { ArrowRightLeft, CheckCircle2, Lock, FileSpreadsheet, ChevronDown } from "lucide-react";

const features = [
  { icon: CheckCircle2, title: "1-to-1 Matching", desc: "Every sale matched to exact purchase lot" },
  { icon: Lock, title: "Per-user Data", desc: "Your data, only visible to you" },
  { icon: FileSpreadsheet, title: "Excel Export", desc: "6 ready-to-use report formats" },
];

const keyTerms = [
  "Your uploaded files are stored privately — no one else can access your data",
  "Subscriptions auto-renew and can be cancelled any time before renewal",
  "We are not liable for errors in reconciliation output — always verify results",
];
          ...
          <div className="mb-5 rounded-xl border border-white/15 bg-white/5 overflow-hidden">
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`w-5 h-5 rounded border-2 flex-shrink-0 flex items-center justify-center transition-all duration-150 ${
                    agreed ? "bg-emerald-500 border-emerald-500" : "bg-white/10 border-white/30"
                  }`}
                ...
                <span className="text-white/70 text-xs font-medium">
                  I agree to the{" "}
                  <span className="text-emerald-400">Terms & Conditions</span>
                </span>
              </div>
              <ChevronDown
            ...
            >
              <div className="px-4 pb-4 border-t border-white/10 pt-3">
                <p className="text-white/40 text-[10px] uppercase tracking-wider font-semibold mb-2">Key points</p>
                <ul className="space-y-2">
                  {keyTerms.map((term, i) => (
                    <li key={i} className="flex items-start gap-2">
                      <div className="w-1 h-1 rounded-full bg-emerald-400/60 mt-1.5 flex-shrink-0" />
                      <span className="text-white/55 text-[11px] leading-relaxed">{term}</span>
                    </li>
                  ))}
                </ul>
                <a
                  href="#"
                  className="text-emerald-400/70 text-[10px] underline underline-offset-2 mt-3 block hover:text-emerald-300"
